...
```{r read-target, message=FALSE, warning=FALSE}
...
```{r fig.alt="SARIMA model fit and forecast", fig.cap="SARIMA model fit and forecast"}
...
```{r fig.alt="Neural network model fit and forecast", fig.cap="Neural network model fit and forecast"}
  ...
  function(days_ahead) {
    epipredict::arx_forecaster(
      ...
      args_list = arx_args_list(
        lags = list(c(0, 7, 14)),
        ahead = days_ahead,
        quantile_levels = signif(c(0.01, 0.025, seq(0.05, 0.95, 0.05), 0.975, 0.99), 2),
        nonneg = TRUE
      )
    )
  }
