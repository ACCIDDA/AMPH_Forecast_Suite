---
title: "Other: Introduction to versioning and nowcasting"
output:
  html_document:
    toc: true
    toc_float: true
...
Let's also load the relevant packages.
...
Moreover, we assume you have followed the first guide of this course, and have an API key ready for the DELPHI Epidata API.
...
epinowcast provides convenient functions to filter a data in the right format. Let's keep only the data that concerns before our forecast date, and let's print the unique report dates
...
We will now do some bookkeeping. The epinowcast function `enw_latest_data()` filters observations to keep only the latest available reported total counts for each reference date. And like we did above, the epinowcast function enw_filter_report_dates() will be used to create a truncated dataset for generating a retrospective nowcast, using the data that would have been available as of the nowcast date.
...
# filter so observed_data and latest_data end on the same day.
...
If you would like to explore nowcasting further, we suggest using a dataset 