...
prep_proj_data <- projection_data_all %>%
  dplyr::mutate(
    reference_date  = dplyr::coalesce(reference_date, origin_date),
...
round_dat <- projection_data %>%
  ...
  dplyr::filter(reference_date == as.Date(curr_origin_date),
...
scoring_df <- dplyr::left_join(
  ...
  dplyr::filter(quantile_level %in% c(0.025, 0.1, 0.5, 0.9, 0.975))
